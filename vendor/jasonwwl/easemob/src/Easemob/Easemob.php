...
class Easemob
{
	private $client_id = "YXA65L76MI80EeSFmAm35LM7Ng";
	private $client_secret = "YXA6EnzDw5itDlCCwoAG2cPYNi74Q08";
	private $org_name = "zizbi";
	private $app_name = "xiaobao";
	private $url;
	...
	public function accreditRegister($options) {
		...
		//Yii::trace($header, 'login\register');
	...
	private function postCurl($url, $option, $header = 0, $type = 'POST') {
		Yii::trace($url, 'login\register');
		Yii::trace($option, 'login\register');
		Yii::trace($header, 'login\register');
		Yii::trace($type, 'login\register');
